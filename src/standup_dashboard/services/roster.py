...
def load(db: Database) -> None:
    ...
    additions = tuple(
        config.EngineerConfig(name=name, email=email, region_keys=(region,))
        for name, email, region in db.get_roster_additions()
    )
    config.rebuild_roster(additions, db.get_region_overrides())


def add_engineer(db: Database, name: str, email: str, region: str, now: datetime) -> None:
    name = (name or "").strip()
    ...
    db.add_roster_engineer(name, email, region, now)
    load(db)
